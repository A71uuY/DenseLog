...
import os
import time
from multi_constraints.multi_cons_exp1 import *

class FarmEnvOri(Env):
    ...
    def __init__(self, training=True, constrained=1, output_dir='output',min_lag=1,max_lag=20,alg='dcrl'):
        ...
        self.state_cons_map = [[] for _ in range(len(self.drone_density.flatten()))]
        self.constraints = np.array(self.init_cons())
        self.max_lag=max_lag
        self.min_lag=min_lag
        self.alg=alg
    
    def _create_density_map(self, state_id, phis_current):
        # Create the mapping from state to the id of constraints it involved in
        self.state_cons_map[state_id].append(len(phis_current)-1)
        return 1
    
    def init_cons(self):
        # We want the same constraint as in DCRL, but by MDCRL
        min_dens = self.drone_density_min.flatten()
        max_dens = self.drone_density_max.flatten()
        n_states = len(self.drone_density.flatten())
        # rho(s)>dron_min -> -1*rho(s)+dron_min <= 0
        # rho(s)<dron_max -> 1*rho(s)-dron_max <= 0
        # final_phi = None
        phis = []
        for state_id in range(n_states):
            min_A, max_A, min_B, max_B = np.zeros(n_states),np.zeros(n_states),np.zeros(n_states),np.zeros(n_states)
            min_den = min_dens[state_id]
            max_den = max_dens[state_id]
            min_A[state_id] = -1
            max_A[state_id] = 1
            min_B[state_id] = min_den
            max_B[state_id] = -max_den
            min_density_atom = Atomic(n_states,min_A,min_B)
            max_density_atom = Atomic(n_states,max_A,max_B)
            phi1,phi2 = AtomFormula(min_density_atom),AtomFormula(max_density_atom)
            # if final_phi is None:
                # final_phi = AndFormula(phi1,phi2)
            # else:
            phis.append(phi1)
            self._create_density_map(state_id,phis)
            phis.append(phi2)
            self._create_density_map(state_id,phis)
        return phis
    ...
    def step(self, a):
        ...
        rew2 = self._add_drone_density_reward_pdl(self.s, rew2)
        if abs(rew - rew2) > 0.01:
            print("Algorithm fault occur!")
            
        rew = self._add_velocity_density_reward(self.v, rew)
        rew2 = self._add_velocity_density_reward(self.v, rew2)
    ...
    def _add_drone_density_reward_pdl(self, s, reward):
        if self.constrained == 0 or not self.training:
            return reward
        x, y = s
        # Check the used densities, get past range and take mean
        x_min, x_max = self._get_past_range(x, y)
        density = self.drone_density[int(y), x_min:x_max+1]
        if density.shape[0] == 0:
            return reward
        
        # Since each constraint only involves one state, we iterate the correlated constraints only.
        sid = int(y)*self.GRID_SIZE+int(x)
        used_cons_id = self.state_cons_map[sid]
        used_cons = [self.constraints[_] for _ in used_cons_id]
        # Check the used lagrange values
        lags = [cons.get_multiplier(int(y)*self.GRID_SIZE+int(x)) for cons in used_cons] # lag_min and lag_max

        # Check density reward and density cost below
        den_mins= []
        cons_past_id = self.state_cons_map[int(y)*self.GRID_SIZE+int(x_min):int(y)*self.GRID_SIZE+int(x_max+1)]
        min_cons_id = [_[0] for _ in cons_past_id]
        max_cons_id = [_[1] for _ in cons_past_id]
        min_cons = self.constraints[min_cons_id]
        max_cons = self.constraints[max_cons_id]
        den_mins = np.array([np.max(c.alpha.b) for c in min_cons]) # We have only non-neg values in b, and the max should be the corresponding value
        v1 = [-c.get_val(self.drone_density.flatten()) for c in min_cons]
        den_min_vals = np.array(v1)
        v2 = [-c.get_val(self.drone_density.flatten()) for c in max_cons]
        den_max_vals = np.array(v2)

        # Check added reward
        rew_min = np.mean(den_mins) + np.mean(np.minimum(0,den_min_vals)) # Give more reward if the constraint is yet to be satisfied
        rew_max = np.mean(np.minimum(0,den_max_vals))
        if self.constrained == 1:
            if np.sum(den_mins) == 0:
                add_term = rew_max*lags[1]
            else:
                add_term = rew_min*lags[0]+rew_max*lags[1]
            return reward+add_term
        else:
