...
class FairGamble(gym.Env):
    ...
    def init_pdl_obj(self):
        # we want low > 40 or high < 5
        A_high = np.zeros(len(self.all_states))
        A_low = np.zeros(len(self.all_states))
        for i in range(len(self.all_states)):
            if self.all_states[i] == 1: # low risk game
                A_low[i]=-1
            if self.all_states[i] == 3: # high risk game
                A_high[i]=1
        all_low = ConsFormula(len(self.all_states),A_low,-40)
        all_high = ConsFormula(len(self.all_states),A_high,5)
        return OrFormula(all_low,all_high)
