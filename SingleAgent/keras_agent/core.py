...
from keras_agent.callbacks import (
    CallbackList,
    TestLogger,
    TrainEpisodeLogger,
    TrainIntervalLogger,
...
class Agent(object):
    ...
    def test(self, env, nb_episodes=1, action_repetition=1, callbacks=None, visualize=True,
             nb_max_episode_steps=None, nb_max_start_steps=0, start_step_policy=None, verbose=1, rec_density_loss = 1):
        ...
        # Arguments
            ...
            nb_max_episode_steps (integer): Number of steps per episode that the agent performs before
                automatically resetting the environment. Set to `None` if each episode should run
                (potentially indefinitely) until the environment signals a terminal state.
            rec_density_loss: If 1, record the density loss, otherwise record rewards only.
        ...
        for episode in range(nb_episodes):
            ...
            self.forward(observation)
            self.backward(0., terminal=False)
            if rec_density_loss and env.constraints is not None:
                episode_cons_val = env.return_cons_val()
                all_episode_consvals.append(episode_cons_val)
            # Report end of episode.
            all_episode_rewards.append(episode_reward)
            if not rec_density_loss:
                episode_logs = {
                    'episode_reward': episode_reward,
                ...
                episode_logs = {
                'episode_consval': episode_cons_val,
                'episode_reward': episode_reward,
                'nb_steps': episode_step,
            }
            callbacks.on_episode_end(episode, episode_logs)
        print('Mean episode reward: {:.3f}'.format(np.mean(all_episode_rewards)))
        print('Mean episode constrant value: {:.3f}'.format(np.mean(all_episode_consvals)))
