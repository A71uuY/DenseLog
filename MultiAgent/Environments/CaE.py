...
class CaE(gym.Env):
    ...
    def __init__(self, n_agents=3,mod_rew=False) -> None:
        ...
        self.pdl_objective = self.init_pdl_obj()
        self.disj_obj1 = self.disj1_obj()
        self.disj_obj2 = self.disj2_obj()
    ...
    def init_pdl_obj(self):
        # we want aout < 10 or b+cout > 20
        A_out = np.zeros(len(self.all_states))
        bcout = np.zeros(len(self.all_states))
        for i in range(len(self.all_states)):
            if self.all_states[i][0] == 1: # aout < 10
                A_out[i]=1
            if self.all_states[i][1] == 1 or self.all_states[i][2] == 1: # b or cout > 20
                bcout[i]=-1
        aout_formula = ConsFormula(len(self.all_states),A_out,0) # a<0
        bcout_formula = ConsFormula(len(self.all_states),bcout,40) # -borc < 20
        return OrFormula(aout_formula,bcout_formula)
    ...
    def disj1_obj(self):
        ...
        for i in range(len(self.all_states)):
            ...
            if self.all_states[i][1] == 1 or self.all_states[i][2] == 1: # b or cout > 20
                bcout[i]=-1
        aout_formula = ConsFormula(len(self.all_states),A_out,0)
        bcout_formula = ConsFormula(len(self.all_states),bcout,40)
        return aout_formula
    ...
    def disj2_obj(self):
        ...
        for i in range(len(self.all_states)):
            ...
            if self.all_states[i][1] == 1 or self.all_states[i][2] == 1: # b or cout > 20
                bcout[i]=-1
        aout_formula = ConsFormula(len(self.all_states),A_out,0)
        bcout_formula = ConsFormula(len(self.all_states),bcout,40) #-bc
        return bcout_formula
