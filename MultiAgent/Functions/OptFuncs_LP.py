...
def pdl_risk(occupancy_measure, environment):
    occupancy_measure = occupancy_measure.reshape(environment.possible_states,
                                                  environment.joint_act_nums)
    density_vec = occupancy_measure.sum(1)
    # or_formula = environment.pdl_objective
    # or_formula.get_val(density_vec)
    risk = environment.pdl_objective.get_val(density_vec)
    return risk

def disj_risk1(occupancy_measure, environment):
    occupancy_measure = occupancy_measure.reshape(environment.possible_states,
                                                  environment.joint_act_nums)
    density_vec = occupancy_measure.sum(1)
    # or_formula = environment.pdl_objective
    # or_formula.get_val(density_vec)
    risk = environment.disj_obj1.get_val(density_vec)
    return risk

def disj_risk2(occupancy_measure, environment):
    occupancy_measure = occupancy_measure.reshape(environment.possible_states,
                                                  environment.joint_act_nums)
    density_vec = occupancy_measure.sum(1)
    # or_formula = environment.pdl_objective
    # or_formula.get_val(density_vec)
    risk = environment.disj_obj2.get_val(density_vec)
    return risk
"""
Constraints below
"""
...
def calculate_regret(occupancy_measure, state_num, joint_act_num, q_table,
    ...
    regret_val = np.zeros(state_num)
    for state_i in range(len(occupancy_measure)):  # For each state
        real_Qs = np.array([q_table[state_i][act_ids] for act_ids in ori_acts
    ...
    for i in range(len(ori_idx)):  # For each possible action
        ...
        for possible_alter_act in alter_acts:  # How many possible alternative action pairs
            regret_val = calculate_regret(occupancy_measure, state_num, joint_act_num,
                             q_table, ori_act, possible_alter_act)
...
def bellmanflow_constraint(occupancy_measure, state_n, jact_n, tranm):
    ...
    bellman_flow_vals = np.zeros(state_n)
    ...
    for row_id in range(len(occupancy_measure)):
        ...
        sum_transition_rho = np.sum(np.multiply(tran_to_row,
                                                occupancy_measure))
