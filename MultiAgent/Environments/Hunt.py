...
class Hunt(gym.Env):
    ...
    def init_pdl_obj(self):
        ...
        A_in = np.zeros(len(self.all_states))
        for i in range(len(self.all_states)):
            if np.sum(self.all_states[i]) == 3: # all_out
                A_out[i]=1
            if np.sum(self.all_states[i]) == 0: # all_in
                A_in[i]=-1
        all_out = ConsFormula(len(self.all_states),A_out,-5)
        all_in = ConsFormula(len(self.all_states),A_in,40)
        return OrFormula(all_in,all_out)
...
if __name__ == '__main__':
    ...
    for step in range(100):
        ...
        env.pdl_objective.gev_val(None)
