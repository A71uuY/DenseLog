...
def generate_trace(env,policy,steps):
    s = env.reset()
    trace = []
    for t in range(steps):
        state_id = env.vec_to_ind(s)
        ...
        action = env.ind_to_act(action_id)
        s_new, r, done, info = env.step(action)
        trace.append(env.vec_to_ind(s_new))
        s = s_new
    return trace
...
def main(obj_name, algo_name, env_name, task_name, constraint_threshold=0.05):
    ...
    if 'PDL' in task_name:
        csv_header = [
            'Environment', 'Task', 'SolutionConcept', 'Method', 'Epoch',
            'Iteration', 'Time','Risk', 'ExpectedReward', 'BFViolated', 'RegretViolated',
            'observe_convergence','check_CE','Phi1Violated','Phi2Violated'
        ]
    ...
    for epc in range(int(arglist.num_runs)):  # Each experiment
        ...
        if task_name == 'MDCE':
            ...
            if obj_name == 'DBCE':
                ...
                obj_func = cal_neg_total_reward
        elif task_name == 'PDL_disj':
            arg_used = env
            obj_func = pdl_risk
        elif task_name == 'disj_obj1':
            arg_used = env
            obj_func = disj_risk1
        elif task_name == 'disj_obj2':
            arg_used = env
            obj_func = disj_risk2
        ...
        for iter in range(int(arglist.max_iter)):
            ...
            if task_name == 'MDCE':
                ...
                result = pdl_risk(om,env)
            elif task_name == 'disj_obj1':
                result = disj_risk1(om,env)
            elif task_name == 'disj_obj2':
                result = disj_risk2(om,env)
            ...
            if task_name == 'PDL_disj':
                density_vec = om.reshape(env.possible_states,
                                                  env.joint_act_nums)
                density_vec = density_vec.sum(1)
                env.pdl_objective.get_max_vio(density_vec)
            policy.derive_pi(
            ...
            if 'PDL' not in task_name:
                if algo_name != 'ModRew':
                    ...
                    if dif_qts + dif_om <= stop_threshold:  # or dif_om <= om_stop_thres:
                        ...
                        f.close()
                        #break
            #np.save(env_name+task_name+obj_name+'iter1.npy',om)
            else:
                if 'PDL' in task_name:
                    density_vec = om.reshape(env.possible_states,
                                                    env.joint_act_nums)
                    density_vec = density_vec.sum(1)
                    v1 = env.pdl_objective.phi1.get_val(density_vec)
                    v2 = env.pdl_objective.phi2.get_val(density_vec)
                    writer.writerow([
                                env_name, task_name, obj_name, algo_name,
                                str(epc),
                                str(iter),
...
if __name__ == "__main__":
    ...
    # main('DBCE','DBCE','FairGamble','disj_obj2')
    main('DBCE','DBCE','CaE','PDL_disj')
    main('DBCE','DBCE','CaE','disj_obj1')
    main('DBCE','DBCE','CaE','disj_obj2')
    # main('DBCE','DBCE','Hunt','PDL_disj')
    # main('DBCE','DBCE','Hunt','disj_obj1')
    # main('DBCE','DBCE','Hunt','disj_obj2')
